...
app.get("/api/get", (req,res) =>{
    ...
    const stmt ="SELECT * FROM books ORDER BY bname;";
...
app.get("/api/getsearch/:bname", (req,res) =>{
    ...
    db.query(`SELECT * FROM books where bname like '%${bname}%' ORDER BY bname; `,  (err,result)=>{
        res.send(result);
        console.log(result);
       console.log(err);
    })
...
app.post("/api/postlike", (req,res) =>{
    ...
    const stmt =`INSERT INTO likes (senuid, recuid, accepted, bookid, bname, price, email) VALUES (?,?,0,?,?,?,"");`;
...
app.put("/api/postlikeacceptemail", (req,res) =>{
    const senuid=req.body.senuid;
    const recuid=req.body.recuid;
    const bookid=req.body.bookid;
    const email=req.body.mail; 

    db.query("Update likes set email=? where senuid=? and recuid=? and bookid=?", [email,senuid,recuid,bookid], (err,result)=>{
    res.send(result);   
    console.log(result);
    console.log(err);
    })
...
app.post("/api/like", (req,res) =>{
    ...
    const bname=req.body.bname;
    const price=req.body.price;
    let rdate = new Date().toISOString().slice(0, 10)
    
    
    const stmt =`INSERT INTO requests (recuid,senuid,bookid,bname,price,rdate) VALUES (?,?,?,?,?,?);`;
    db.query(stmt, [recuid,senuid,bookid,bname,price,rdate], (err,result)=>{
   //     console.log(result);
    })
...
app.post("/api/insert", (req,res) =>{
    ...
    const stmt =`INSERT INTO users (uname, uid, password, email) VALUES (?,?,?,?);`;
    db.query(stmt, [name,uid,key,email], (err,result)=>{
     console.log(result);
    })
